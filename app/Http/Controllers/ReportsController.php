...
class ReportsController extends Controller
{
  ...
  public function monthly()
  {
    ...
    $page_settings['seltab'] = 'reports';
    $page_settings['seltab2'] = 'monthly';
    ...
    return view('reports.monthly', ['user' => $user, 'page_settings'=> $page_settings, 'status' => $this->status, 'months' => $this->months, 'page_title' => 'Monthly Reports']);
  }
  ...
  public function yearly()
  {
    ...
    $page_settings['seltab'] = 'reports';
    $page_settings['seltab2'] = 'yearly';

    if(request()->ajax()){

      $year = (isset($_GET['year']) ? $_GET['year'] : 0);

      $dbtable = Invoices::with('items','client','project')->whereYear('created_at', $year)->get();
      return datatables()->of($dbtable)
    ...
    return view('reports.yearly', ['user' => $user, 'page_settings'=> $page_settings, 'status' => $this->status, 'months' => $this->months, 'page_title' => 'Yearly Reports']);
  }

  public function printyearly($y)
  {
    $user = auth()->user();
    
    $dbtable = Invoices::with('items','client','project')->whereYear('created_at', $y)->get();

    if(count($dbtable) != 0){

      $page_title = $y . ' Yearly Report ';
      ...
      return view('reports.printyearly', ['user' => $user, 'page_settings'=> $this->page_settings, 'status' => $this->status, 'page_title' => $page_title, 'y' => $y, 'months' => $this->months]);
    }else{
      return notifyRedirect($this->homeLink, 'Invalid dates', 'danger');
    }
  }
